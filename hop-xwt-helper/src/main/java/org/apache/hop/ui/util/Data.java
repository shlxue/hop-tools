...
final class Data {
  ...
  public static <T> void setValue(Widget widget, T value) {
    Consumer<T> setter = getSetter(widget, (Class<T>) value.getClass(), UI_EDITOR_SETTER);
  ...
  static <T> Optional<T> tryGet(Widget widget, Class<T> clazz, String key) {
    ...
    if (value != null) {
      Assert.isAssignable(clazz, value.getClass(), () -> "Wrong data type for key " + key);
  ...
  static <T> Consumer<T> getSetter(Widget widget, Class<T> clazz, String key) {
    return (Consumer) get(widget, Consumer.class, key);
  }
  ...
  static void add(Widget widget, String key, Object value) {
    Assert.isNull(widget.getData(key), () -> "Already exist data for key " + key);

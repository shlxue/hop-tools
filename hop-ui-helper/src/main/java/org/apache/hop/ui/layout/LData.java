...
public final class LData {
  ...
  public static FormData fill(Control top, Control bottom) {
    return fill(top, bottom, true);
  }

  public static FormData fill(Control top, Control bottom, boolean hMiddle) {
    FormDataFactory factory = form().left(hMiddle ? MIDDLE : 0).right(100);
    if (top != null) {
      factory.top(top);
    } else {
      factory.top(0);
  ...
  //// GridData for Dialog
  public static GridDataFactory toEnd() {
    return grid(GridData.HORIZONTAL_ALIGN_END);
  }
